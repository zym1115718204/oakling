...
class Query(object):
    ...
    def query_local_system_status(self):
        ...
        # todo get local file system config
        ...
        if settings.HDFS_NAMENODE_HOST and settings.HDFS_NAMENODE_PORT and settings.HDFS_DATAFILE_DIRS:
            ...
            try:
                ...
                if _modification_time:
                    _modification_time = arrow.get(_modification_time).format('YYYY-MM-DD HH:mm:ss')
            ...
            _hdfs_config["length"] = float(count.get("length", 0))/1000/1024/1024  # yte =》GB
            _hdfs_config["spaceConsumed"] = float(count.get("spaceConsumed", 0))/1000/1024/1024  # yte =》GB
            ...
            """
            try:
                for path in client.du([root_dir]):
                    project = {}
